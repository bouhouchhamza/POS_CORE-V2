...
- Build the Windows installer with `npm run build:production-installer -w @bimik/desktop`; a release build without the production public verification key must fail.
- On a clean Windows PC, activate without PowerShell environment injection, close all Bimik processes, disconnect networking, reopen from the normal shortcut, login and use licensed POS modules.
