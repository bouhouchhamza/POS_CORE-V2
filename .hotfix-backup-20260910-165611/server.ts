...
const verifyPassword = (hash: string, password: string) => hash.startsWith("$2") ? bcrypt.compare(password, hash) : Promise.resolve(false);
...
export async function buildLocalApp(options: LocalServerOptions = {}) {
  ...
  const localForm = async (request: FastifyRequest, folder: "products" | "categories") => {
    ...
    for await (const part of request.parts()) {
      ...
      const extension = imageExtensions.get(part.mimetype); if (!extension) throw Object.assign(new Error("Lâ€™image doit Ãªtre JPG, PNG ou WebP."), { statusCode: 422 });
      const bytes = await part.toBuffer(); if (!validImage(bytes, part.mimetype)) throw Object.assign(new Error("Le contenu de lâ€™image est invalide."), { statusCode: 422 });
  ...
  const authenticate = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.code(401).send({ message: "Non authentifiÃ©." });
    }
  };
  ...
  app.setErrorHandler((error: unknown, _request, reply) => { if (error instanceof z.ZodError) return reply.code(422).send({ message: error.issues[0]?.message ?? "DonnÃ©es invalides.", errors: error.flatten().fieldErrors }); const candidate = error as { statusCode?: number; message?: string }; const code = Number(candidate.statusCode); if (code >= 400 && code < 500) return reply.code(code).send({ message: candidate.message ?? "RequÃªte invalide." }); app.log.error(error); return reply.code(500).send({ message: "Une erreur interne est survenue." }); });
  ...
  app.post("/api/logout", async (request, reply) => { const raw = request.cookies.bimik_refresh; if (raw) db.prepare("UPDATE refresh_tokens SET revoked_at=? WHERE token_hash=?").run(now(), tokenHash(raw)); reply.clearCookie("bimik_refresh", { path: "/api" }); return { message: "DÃ©connexion rÃ©ussie." }; });
  ...
  app.get("/api/categories/:id", { preHandler: authenticate }, async (request, reply) => { const row = queryOne(db, "SELECT * FROM categories WHERE id=?", Number((request.params as any).id)); return row ? { data: { ...row,is_public:Boolean(row.is_public), image_url: row.image } } : reply.code(404).send({ message: "CatÃ©gorie introuvable." }); });
  app.post("/api/categories", { preHandler: patronOnly }, async (request) => { const form = await localForm(request, "categories"); try { const input = categorySchema.parse(form.fields); const timestamp = now(); const result = db.prepare("INSERT INTO categories(name,image,is_public,created_at,updated_at) VALUES(?,?,?,?,?)").run(input.name, form.image,Number(input.is_public??true), timestamp, timestamp); return { data: queryOne(db, "SELECT * FROM categories WHERE id=?", Number(result.lastInsertRowid)) }; } catch (error) { if (form.image) fs.rmSync(path.join(paths.uploads, form.image.replace(/^\/uploads\//, "")), { force: true }); throw error; } });
  const updateCategoryHandler = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!current) return reply.code(404).send({ message: "CatÃ©gorie introuvable." });
  ...
  app.delete("/api/categories/:id", { preHandler: patronOnly }, async (request, reply) => { const id = Number((request.params as any).id); const current = queryOne(db, "SELECT * FROM categories WHERE id=?", id); if (!current) return reply.code(404).send({ message: "CatÃ©gorie introuvable." }); immediate(db, () => { db.prepare("UPDATE products SET category_id=NULL,updated_at=? WHERE category_id=?").run(now(), id); db.prepare("DELETE FROM categories WHERE id=?").run(id); }); removeUnreferencedImage(current.image); return { message: "CatÃ©gorie supprimÃ©e." }; });
  ...
  app.post("/api/products", { preHandler: productManager }, async (request) => { const form = await localForm(request, "products"); try { const input = productSchema.parse(multipartProduct(form.fields)); const timestamp = now(); const result = db.prepare("INSERT INTO products(category_id,name,sku,barcode,unit,tax_rate,purchase_price_cents,sale_price_cents,stock,min_stock,track_stock,image,is_active,is_public,available,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(input.category_id ?? null, input.name,input.sku||null,input.barcode||null,input.unit,input.tax_rate, cents(input.purchase_price), cents(input.sale_price), input.stock, input.min_stock, Number(input.track_stock), form.image, Number(input.is_active),Number(input.is_public),Number(input.available), timestamp, timestamp); return { data: safeProduct(queryOne(db, "SELECT p.*,c.name category_name FROM products p LEFT JOIN categories c ON c.id=p.category_id WHERE p.id=?", Number(result.lastInsertRowid))) }; } catch (error) { if (form.image) fs.rmSync(path.join(paths.uploads, form.image.replace(/^\/uploads\//, "")), { force: true }); throw error; } });
  ...
  app.delete("/api/products/:id", { preHandler: patronOnly }, async (request, reply) => {
    ...
    if (history > 0) { db.prepare("UPDATE products SET is_active=0,updated_at=? WHERE id=?").run(now(), id); return { message: "Produit dÃ©sactivÃ© afin de conserver son historique.", data: safeProduct(queryOne(db, "SELECT * FROM products WHERE id=?", id)) }; }
    db.prepare("DELETE FROM products WHERE id=?").run(id);
    removeUnreferencedImage(current.image);
    return { message: "Produit supprimÃ©." };
  });

  const stockOperation = (kind: "increase" | "decrease" | "correction") => async (request: FastifyRequest, reply: FastifyReply) => { const id = Number((request.params as any).id); const input = kind === "correction" ? correctionSchema.parse(request.body) : stockSchema.parse(request.body); return immediate(db, () => { const product = queryOne(db, "SELECT * FROM products WHERE id=?", id); if (!product) return reply.code(404).send({ message: "Produit introuvable." }); if (!product.track_stock) return reply.code(422).send({ message: "Le suivi du stock est dÃ©sactivÃ© pour ce produit." }); const before = Number(product.stock); const after = kind === "correction" ? (input as any).stock : kind === "increase" ? before + (input as any).quantity : before - (input as any).quantity; if (after < 0) return reply.code(422).send({ message: "Stock insuffisant." }); const timestamp = now(); db.prepare("UPDATE products SET stock=?,updated_at=? WHERE id=?").run(after, timestamp, id); db.prepare("INSERT INTO stock_movements(product_id,user_id,type,quantity,before_stock,after_stock,note,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)").run(id, request.user.sub, kind, after - before, before, after, (input as any).note ?? null, timestamp, timestamp); return { data: safeProduct(queryOne(db, "SELECT * FROM products WHERE id=?", id)) }; }); };
  ...
  app.get("/api/cash-register/current", { preHandler: authenticate }, async () => ({ data: mapCashRegisterSession(queryOne(db, "SELECT * FROM cash_register_sessions WHERE status='open' LIMIT 1")) }));
  app.post("/api/cash-register/open", { preHandler: cashManager }, async (request, reply) => { const input = z.object({ opening_cash: z.coerce.number().finite().nonnegative().default(0), opening_note: z.string().trim().max(500).nullable().optional() }).parse(request.body); return immediate(db, () => { if (queryOne(db, "SELECT id FROM cash_register_sessions WHERE status='open'")) return reply.code(422).send({ message: "La caisse est dÃ©jÃ  ouverte." }); const timestamp = now(); const result = db.prepare("INSERT INTO cash_register_sessions(business_date,status,opened_at,opened_by_user_id,opening_cash_cents,opening_note,created_at,updated_at) VALUES(?,'open',?,?,?,?,?,?)").run(localDate(), timestamp, request.user.sub, cents(input.opening_cash), input.opening_note ?? null, timestamp, timestamp); return { data: mapCashRegisterSession(queryOne(db, "SELECT * FROM cash_register_sessions WHERE id=?", Number(result.lastInsertRowid))) }; }); });
  app.post("/api/cash-register/close", { preHandler: cashManager }, async (request, reply) => { const input = z.object({ actual_cash: z.coerce.number().finite().nonnegative(), closing_note: z.string().trim().max(500).nullable().optional() }).parse(request.body); return immediate(db, () => { const session = queryOne(db, "SELECT * FROM cash_register_sessions WHERE status='open'"); if (!session) return reply.code(422).send({ message: "Aucune caisse ouverte." }); const cashSalesTotal = Number(queryOne(db, "SELECT coalesce(sum(total_cents),0) total FROM sales WHERE cash_register_session_id=? AND lower(coalesce(payment_method,'cash'))='cash'", session.id)?.total ?? 0); const expected = Number(session.opening_cash_cents) + cashSalesTotal; const actual = cents(input.actual_cash); db.prepare("UPDATE cash_register_sessions SET status='closed',closed_at=?,closed_by_user_id=?,expected_cash_cents=?,actual_cash_cents=?,difference_cents=?,closing_note=?,updated_at=? WHERE id=?").run(now(), request.user.sub, expected, actual, actual - expected, input.closing_note ?? null, now(), session.id); return { data: mapCashRegisterSession(queryOne(db, "SELECT * FROM cash_register_sessions WHERE id=?", session.id)) }; }); });
  ...
  app.get("/api/reports/monthly", { preHandler: patronOnly }, async (request, reply) => {
    ...
    if (
      ...
      return reply.code(422).send({
        message: "Le mois doit Ãªtre au format AAAA-MM.",
      });
    }
  ...
  const menuMoneyCents = (raw: string | null) => {
    if (typeof raw !== "string") {
      throw Object.assign(
        new Error("Chaque produit importÃ© doit avoir un prix valide."),
        { statusCode: 422 },
      );
    }
  ...
  app.post(
    ...
    async (request, reply) => {
      if (!request.isMultipart()) {
        return reply.code(422).send({
          message: "SÃ©lectionnez un fichier PDF.",
        });
      }
      ...
      if (part.fieldname !== "pdf") {
        ...
        return reply.code(422).send({
          message:
            "Champ de fichier inattendu. Utilisez le champ Â« pdf Â».",
        });
      }
      ...
      if (part.mimetype !== "application/pdf") {
        ...
        return reply.code(422).send({
          message: "Le fichier doit Ãªtre un PDF valide.",
        });
      }
      ...
      try {
        ...
        return reply.code(413).send({
          message: "Le PDF ne doit pas dÃ©passer 5 Mio.",
        });
      }
      ...
      try {
        ...
        if (!parsed.categories.length) {
          return reply.code(422).send({
            message:
              "Aucun produit exploitable nâ€™a Ã©tÃ© trouvÃ© dans ce PDF.",
          });
        }
    ...
    async (request, reply) => {
      ...
      const result = immediate(db, () => {
        ...
        if (session.status === "confirmed") {
          if (!session.result_json) {
            throw new Error(
              "Session confirmÃ©e sans rÃ©sultat enregistrÃ©.",
            );
          }
        ...
        for (
          ...
          if (
            ...
            if (!existing) {
              throw menuBusinessError(
                "La catÃ©gorie existante sÃ©lectionnÃ©e nâ€™existe plus.",
              );
            }
            ...
            if (exact) {
              throw menuBusinessError(
                `La catÃ©gorie Â« ${category.name} Â» existe dÃ©jÃ . Choisissez-la explicitement.`,
              );
            }
          ...
          for (
            ...
            if (
              ...
              if (!existing) {
                throw menuBusinessError(
                  "Le produit sÃ©lectionnÃ© nâ€™existe plus.",
                );
              }
              ...
              if (exact) {
                throw menuBusinessError(
                  `Le produit Â« ${productName} Â» existe dÃ©jÃ . RÃ©solvez explicitement le doublon.`,
                );
              }
      ...
      if (result.kind === "missing") {
        return reply.code(404).send({
          message:
            "Session dâ€™importation introuvable ou expirÃ©e.",
        });
      }
...
function reportForDay(
  ...
  if (
    ...
    throw new Error("Impossible de dÃ©terminer la date locale de Casablanca.");
  }
...
if (entry) startLocalServer().catch((error) => { console.error(error instanceof Error ? error.message : "Le service local n'a pas dÃ©marrÃ©."); process.exitCode = 1; });
