...
export const passwordVerifierSchema=z.string().min(20).max(512).refine(value=>/^\$2[aby]\$\d{2}\$[./A-Za-z0-9]{53}$/.test(value)||/^\$argon2(?:id|i|d)\$v=\d+\$[^$\r\n]+\$[^$\r\n]+\$[^$\r\n]+$/.test(value),'A supported password verifier is required.');
export const commercialCertificateSchema=z.object({version:z.literal(2),certificate_id:z.string().uuid(),license_id:z.string().uuid(),customer_id:z.string().uuid(),vendor_business_id:z.string().uuid(),business_id:z.number().int().positive().nullable(),business_type:businessTypeSchema,plan:z.string().nullable(),features:z.array(z.enum(featureKeys)).max(featureKeys.length).refine(items=>new Set(items).size===items.length),installation_id:z.string().uuid(),device_fingerprint:z.string().regex(/^[a-f0-9]{64}$/),issued_at:z.string().datetime(),expires_at:z.string().datetime().nullable(),offline_validity_days:z.number().int().positive().nullable(),bootstrap:z.object({business:z.object({name:z.string().min(1).max(255),logo:z.string().nullable(),currency:z.string().length(3),locale:z.string().min(2).max(20),timezone:z.string().min(1).max(100)}).strict(),branch:z.object({name:z.string().min(1).max(255),code:z.string().min(1).max(100),address:z.string().nullable(),phone:z.string().nullable()}).strict(),users:z.array(z.object({name:z.string().min(1).max(255),email:z.string().email().max(255),password:passwordVerifierSchema,role:z.enum(roleKeys),is_active:z.boolean()}).strict()).min(1).max(100)}).strict().optional()}).strict();
