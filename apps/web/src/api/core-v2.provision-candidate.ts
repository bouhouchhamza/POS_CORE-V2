...
import type {Business,Customer,FeatureKey,Purchase,PurchaseDetail,PurchaseReturn,RestaurantTable,Room,Supplier,UniversalOrder} from '../types'
export type SetupStatus={configured:boolean;business:Pick<Business,'id'|'name'|'slug'|'logo'|'business_type'>|null}
...
export const provisionBusiness=async(payload:unknown)=>unwrapData<unknown>(await api.post('/provision',payload))
