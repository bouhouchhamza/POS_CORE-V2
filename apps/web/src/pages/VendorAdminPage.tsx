...
type Duration = '1_month'|'3_months'|'6_months'|'1_year'|'custom'
...
const emptyLicense = {vendor_business_id:'',plan_id:'',duration:'1_year' as Duration,custom_expires_at:'',notes:''}
const emptyOnboarding = {customer_name:'',email:'',phone:'',customer_notes:'',business_name:'',business_type:'retail',business_notes:'',plan_id:'',duration:'1_year' as Duration,custom_expires_at:''}
...
const expiryLabel = (value:unknown) => value ? date(value) : 'Sans expiration'
...
function expiryFromDuration(duration:Duration, custom:string){
  ...
  if(duration==='1_month') result.setMonth(result.getMonth()+1)
  if(duration==='3_months') result.setMonth(result.getMonth()+3)
  if(duration==='6_months') result.setMonth(result.getMonth()+6)
  if(duration==='1_year') result.setFullYear(result.getFullYear()+1)
  return result.toISOString()
}
...
export default function VendorAdminPage(){
  ...
  const [renewDuration,setRenewDuration] = useState<Duration>('1_year')
  ...
  async function submitLicense(event:FormEvent){
    ...
    try{
      ...
      const created = await vendorApi.post<Row&{license_key:string;activation?:{expires_at?:string|null;channel?:string}}>('/licenses',{
        ...
        offline_validity_days:selected.offline_validity_days?Number(selected.offline_validity_days):null,
  ...
  async function submitRenew(event:FormEvent){
    ...
    setLoading(true); clearMessages()
    try{
      await vendorApi.post(`/licenses/${renewTarget.id}/renew`,{expires_at:expiryFromDuration(renewDuration,renewCustom)})
  ...
  return <main className="vendor-admin vendor-admin-shell">
    ...
    {modal?<div className="modal-backdrop"><div className="modal-content vendor-form-modal">
      ...
      {modal==='license'?<form onSubmit={submitLicense}><h3>Nouvelle licence</h3><label>Business<select required value={license.vendor_business_id} onChange={e=>setLicense({...license,vendor_business_id:e.target.value})}><option value="">Selectionner</option>{businesses.filter(item=>item.status==='active').map(item=><option key={item.id} value={item.id}>{item.customer_name} - {item.name}</option>)}</select></label><label>Plan<select required value={license.plan_id} onChange={e=>setLicense({...license,plan_id:e.target.value})}><option value="">Selectionner</option>{plans.filter(item=>item.active!==false).map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Duree<select value={license.duration} onChange={e=>setLicense({...license,duration:e.target.value as Duration})}>{(Object.keys(durationLabels) as Duration[]).map(item=><option key={item} value={item}>{durationLabels[item]}</option>)}</select></label>{license.duration==='custom'?<label>Date d'expiration<input required type="datetime-local" value={license.custom_expires_at} onChange={e=>setLicense({...license,custom_expires_at:e.target.value})}/></label>:null}<label>Notes<textarea value={license.notes} onChange={e=>setLicense({...license,notes:e.target.value})}/></label><div className="modal-actions"><button className="button secondary" type="button" onClick={()=>setModal(null)}>Annuler</button><button className="button">Creer la licence</button></div></form>:null}
      {modal==='renew'?<form onSubmit={submitRenew}><h3>Renouveler la licence</h3><p><strong>{renewTarget?.vendor_business_name}</strong> - {renewTarget?.customer_name}</p><label>Nouvelle duree<select value={renewDuration} onChange={e=>setRenewDuration(e.target.value as Duration)}>{(Object.keys(durationLabels) as Duration[]).map(item=><option key={item} value={item}>{durationLabels[item]}</option>)}</select></label>{renewDuration==='custom'?<label>Date d'expiration<input required type="datetime-local" value={renewCustom} onChange={e=>setRenewCustom(e.target.value)}/></label>:null}<div className="modal-actions"><button className="button secondary" type="button" onClick={()=>{setRenewTarget(null);setModal(null)}}>Annuler</button><button className="button">Renouveler</button></div></form>:null}
    </div></div>:null}
