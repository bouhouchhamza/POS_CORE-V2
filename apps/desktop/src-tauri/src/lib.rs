...
fn provision_product_images(
    ...
    std::fs::create_dir_all(&uploads)
        .map_err(|error| format!("Impossible de créer le dossier images: {error}"))?;
    ...
    for asset in product_images_generated::PRODUCT_IMAGES {
        ...
        let target_needs_refresh = if target.is_file() {
            let installed_bytes = std::fs::read(&target).map_err(|error| {
                format!(
                    "Impossible de lire l'image installée pour {} ({}): {error}",
                    asset.product_name,
                    target.display(),
                )
            })?;
        ...
        if target_needs_refresh {
            ...
            let mut file = std::fs::OpenOptions::new()
                ...
                .open(&temporary)
                .map_err(|error| {
                    format!("Impossible de préparer {}: {error}", temporary.display())
                })?;
            ...
            std::fs::copy(&temporary, &target).map_err(|error| {
                format!("Impossible de mettre à jour {}: {error}", target.display())
            })?;
...
fn license_credential_entry() -> Result<keyring::Entry, String> {
    ...
    .map_err(|error| format!("Impossible d'ouvrir le stockage sécurisé Windows: {error}"))
}
...
fn save_license_device_identity(
    ...
    {
        return Err("Identité d'appareil invalide.".into());
    }
    ...
    #[cfg(windows)]
    {
        ...
        let serialized = serde_json::to_string(&stored)
            .map_err(|error| format!("Impossible de sérialiser l'identité: {error}"))?;

        license_credential_entry()?
            .set_password(&serialized)
            .map_err(|error| format!("Impossible d'enregistrer l'identité sécurisée: {error}"))?;
    ...
    {
        let _ = private_key;
        Err("Le stockage sécurisé de licence est disponible uniquement sur Windows.".into())
    }
}
...
fn load_license_device_identity() -> Result<Option<LicenseDeviceIdentity>, String> {
    #[cfg(windows)]
    {
        let raw = match license_credential_entry()?.get_password() {
            ...
            Err(error) => {
                return Err(format!(
                    "Impossible de lire l'identité sécurisée Windows: {error}"
                ))
            }
        };

        let stored: StoredLicenseDeviceIdentity = serde_json::from_str(&raw)
            .map_err(|error| format!("Identité sécurisée invalide: {error}"))?;

        if stored.installation_id.trim().is_empty()
            || stored.public_key.len() < 40
            || stored.private_key.len() < 40
        {
            return Err("Identité sécurisée incomplète.".into());
        }
...
fn sign_license_device_payload(payload: String) -> Result<String, String> {
    ...
    #[cfg(windows)]
    {
        ...
        let raw = license_credential_entry()?
            .get_password()
            .map_err(|error| {
                format!("Identité sécurisée introuvable: {error}")
            })?;

        let stored: StoredLicenseDeviceIdentity =
            serde_json::from_str(&raw)
                .map_err(|error| {
                    format!("Identité sécurisée invalide: {error}")
                })?;

        let private_jwk: Ed25519PrivateJwk =
            serde_json::from_str(&stored.private_key)
                .map_err(|error| {
                    format!("Clé privée Ed25519 invalide: {error}")
                })?;

        let public_jwk: Ed25519PublicJwk =
            serde_json::from_str(&stored.public_key)
                .map_err(|error| {
                    format!("Clé publique Ed25519 invalide: {error}")
                })?;
        ...
        {
            return Err("Format de clé Ed25519 invalide.".into());
        }

        let private_bytes = URL_SAFE_NO_PAD
            .decode(private_jwk.d.as_bytes())
            .map_err(|_| "Clé privée Ed25519 invalide.".to_string())?;

        let private_bytes: [u8; 32] = private_bytes
            .try_into()
            .map_err(|_| "Longueur de clé privée Ed25519 invalide.".to_string())?;

        let signing_key = SigningKey::from_bytes(&private_bytes);

        let expected_public = URL_SAFE_NO_PAD
            .decode(public_jwk.x.as_bytes())
            .map_err(|_| "Clé publique Ed25519 invalide.".to_string())?;

        if expected_public.as_slice()
            != signing_key.verifying_key().as_bytes()
        {
            return Err(
                "La clé privée ne correspond pas à la clé publique.".into(),
            );
        }
    ...
    #[cfg(not(windows))]
    {
        Err("La signature matérielle est disponible uniquement sur Windows.".into())
    }
}
...
mod windows_touch_keyboard {
    ...
    pub fn show() -> Result<(), String> {
        ...
        let mut last = requests.lock().map_err(|_| {
            "Le clavier tactile Windows est momentanément indisponible.".to_string()
        })?;
...
mod tests {
    ...
    fn cp850_preserves_french_accents_and_initializes_printer() {
        let bytes = cp850_receipt("Café réglé");
    ...
    fn bundled_product_images_are_idempotent_and_preserve_custom_images() {
        ...
        connection
            .execute(
                "UPDATE products SET name='Nom approuvé renommé',image=? WHERE id=?",
                rusqlite::params![
                    format!(
