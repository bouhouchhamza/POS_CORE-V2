...
export function registerLocalCoreV2Routes(app:FastifyInstance,db:DatabaseSync,authenticate:(r:FastifyRequest,p:FastifyReply)=>Promise<unknown>){
  ...
  app.post('/api/sync/cash-register/conflict',{preHandler:guard()},async(r,p)=>{const u=user(r),input=z.object({client_id:z.string().uuid(),local_session_id:z.number().int().positive()}).parse(r.body);tx(()=>{db.prepare("update cash_register_sessions set sync_status='conflict',updated_at=? where id=? and business_id=?").run(now(),input.local_session_id,u.business_id);db.prepare("update sync_mutations set sync_status='conflict',last_error='CASH_REGISTER_ALREADY_OPEN',updated_at=? where business_id=? and client_id=?").run(now(),u.business_id,input.client_id)});return{message:'Conflict recorded.'}});
