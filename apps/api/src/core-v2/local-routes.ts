...
export function registerLocalCoreV2Routes(app:FastifyInstance,db:DatabaseSync,authenticate:(r:FastifyRequest,p:FastifyReply)=>Promise<unknown>){
  ...
  app.get('/api/setup/status',async(request)=>{const business=one(db,'select id,name,slug,logo,business_type from businesses order by id limit 1'),state=licenseState();const active=state?.status==='active';let authenticated=false;if(request.headers.authorization){try{await request.jwtVerify();const current=one(db,'select u.role,u.is_active from users u join refresh_tokens r on r.user_id=u.id where u.id=? and r.id=? and r.revoked_at is null',request.user.sub,request.user.sid);authenticated=Boolean(current?.is_active&&current.role===request.user.role)}catch{authenticated=false}}return {data:{state:active?(business?(authenticated?'READY':'DEVICE_ACTIVATED'):'SETUP_REQUIRED'):'READY_FOR_ACTIVATION',configured:Boolean(business),requires_license_activation:!active,requires_provisioning:false,business:business??null}}});
