...
export function registerLocalCoreV2Routes(app:FastifyInstance,db:DatabaseSync,authenticate:(r:FastifyRequest,p:FastifyReply)=>Promise<unknown>){
  ...
  app.get('/api/sync/outbox',{preHandler:guard()},async r=>({data:all(db,"select * from sync_mutations where business_id=? and entity_type not in ('branches','settings','categories','units','products','product_variants','product_modifiers','customers','suppliers','users') and sync_status in('pending','failed') order by id limit 100",user(r).business_id).map(x=>({...x,payload:JSON.parse(x.payload_json)}))}));
