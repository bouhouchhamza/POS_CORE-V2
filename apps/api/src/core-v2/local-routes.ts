...
export function registerLocalCoreV2Routes(app:FastifyInstance,db:DatabaseSync,authenticate:(r:FastifyRequest,p:FastifyReply)=>Promise<unknown>){
  ...
  app.patch('/api/table-events/:id/resolve',{preHandler:guard('tables')},async(r,p)=>{const u=user(r),eventId=Number((r.params as any).id),stamp=now(),result=db.prepare("update table_events set status='resolved',resolved_by_user_id=?,resolved_at=? where id=? and business_id=? and status='pending'").run(u.id,stamp,eventId,u.business_id);if(!result.changes)return p.code(404).send({message:'Request not found.'});const ev=one(db,'select * from table_events where id=?',eventId);db.prepare("insert into sync_mutations(business_id,client_id,entity_type,entity_id,operation,payload_json,sync_status,created_at,updated_at) values(?,?, 'table_event',?,'resolve',?,?,?,?)").run(u.business_id,ev.client_id??crypto.randomUUID(),String(eventId),JSON.stringify({event_client_id:ev.client_id,status:'resolved'}),'pending',stamp,stamp);return {data:ev}});
