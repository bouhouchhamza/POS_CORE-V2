...
export function registerLocalCoreV2Routes(app:FastifyInstance,db:DatabaseSync,authenticate:(r:FastifyRequest,p:FastifyReply)=>Promise<unknown>){
  const user=(r:FastifyRequest)=>one(db,'select * from users where id=?',r.user.sub),businessWide=(u:any)=>['patron','owner','admin','manager'].includes(u.role);
  ...
  const developmentSetup=()=>process.env.LICENSE_MODE==='development'&&process.env.NODE_ENV!=='production';
  const bootstrapCertificate=(certificate:Extract<LicenseCertificate,{version:2}>)=>{
    if(one(db,'select id from businesses limit 1')||!certificate.bootstrap)return;
    const source=certificate.bootstrap,stamp=now();
    const slug=`licensed-${certificate.vendor_business_id.slice(0,8)}`;
    const created=db.prepare("insert into businesses(name,slug,business_type,logo,currency,locale,timezone,tax_settings_json,receipt_settings_json,vendor_business_id,created_at,updated_at) values(?,?,?,?,?,?,?,'{}','{}',?,?,?)").run(source.business.name,slug,certificate.business_type,source.business.logo,source.business.currency,source.business.locale,source.business.timezone,certificate.vendor_business_id,stamp,stamp);
    const businessId=Number(created.lastInsertRowid);
    const branch=db.prepare('insert into branches(business_id,name,code,address,phone,active,created_at,updated_at) values(?,?,?,?,?,1,?,?)').run(businessId,source.branch.name,source.branch.code,source.branch.address,source.branch.phone,stamp,stamp);
    const branchId=Number(branch.lastInsertRowid);
    for(const feature of certificate.features)db.prepare('insert into business_features(business_id,feature,created_at) values(?,?,?)').run(businessId,feature,stamp);
    for(const profile of source.users)db.prepare('insert into users(business_id,branch_id,name,email,password,role,is_active,created_at,updated_at) values(?,?,?,?,?,?,?,?,?)').run(businessId,branchId,profile.name,profile.email.toLowerCase(),profile.password,profile.role,Number(profile.is_active),stamp,stamp);
  };
  ...
  app.get('/api/setup/status',async(request)=>{const business=one(db,'select id,name,slug,logo,business_type from businesses order by id limit 1'),state=licenseState(),certificate=storedCertificate(state);const active=state?.status==='active'&&certificate&&certificateStatus(certificate)==='active';let authenticated=false;if(request.headers.authorization){try{await request.jwtVerify();const current=one(db,'select u.role,u.is_active from users u join refresh_tokens r on r.user_id=u.id where u.id=? and r.id=? and r.revoked_at is null',request.user.sub,request.user.sid);authenticated=Boolean(current?.is_active&&current.role===request.user.role)}catch{authenticated=false}}const stateName=developmentSetup()&&!business?'SETUP_REQUIRED':active?(business?(authenticated?'READY':'DEVICE_ACTIVATED'):'LOCAL_BOOTSTRAP_REQUIRED'):'READY_FOR_ACTIVATION';return {data:{state:stateName,configured:Boolean(business),requires_license_activation:!active,requires_provisioning:false,business:business??null}}});
  app.post('/api/setup',async(r,p)=>{
    if(!developmentSetup())return p.code(403).send({message:'Manual setup is available only in local development or explicit recovery.',code:'SETUP_DISABLED_IN_PRODUCTION'});
  ...
  app.addHook('preHandler',async(r,p)=>{if(r.method!=='PATCH'||!/^\/api\/orders\/\d+\/status$/.test(r.url.split('?')[0])||(r.body as any)?.status!=='cancelled')return;await guard('pos')(r,p);if(p.sent)return;const u=user(r),orderId=Number((r.params as any).id),current=one(db,'select * from orders where id=? and business_id=? and (? or branch_id is ?)',orderId,u.business_id,Number(businessWide(u)),u.branch_id);if(!current)return p.code(404).send({message:'Order not found.'});if(current.payment_status==='paid'||['completed','cancelled'].includes(current.status))return p.code(409).send({message:'Paid, completed, or cancelled orders cannot change status.'});const stamp=now();tx(()=>{for(const line of all(db,'select oi.product_id,oi.quantity,p.stock,p.track_stock,p.name from order_items oi join products p on p.id=oi.product_id where oi.order_id=? and p.business_id=?',orderId,u.business_id)){if(!line.track_stock)continue;const before=Number(line.stock),after=before+Number(line.quantity);db.prepare('update products set stock=?,updated_at=? where id=? and business_id=?').run(after,stamp,line.product_id,u.business_id);db.prepare("insert into stock_movements(business_id,branch_id,product_id,user_id,type,quantity,before_stock,after_stock,note,created_at,updated_at) values(?,?,?,?,'order_cancel',?,?,?,?,?,?)").run(u.business_id,current.branch_id,line.product_id,u.id,Number(line.quantity),before,after,`Cancelled order ${current.order_number}`,stamp,stamp)}db.prepare("update orders set status='cancelled',updated_at=? where id=? and business_id=?").run(stamp,orderId,u.business_id);if(current.table_id)db.prepare("update restaurant_tables set status='available',updated_at=? where id=? and business_id=?").run(stamp,current.table_id,u.business_id);db.prepare("insert into audit_logs(business_id,branch_id,user_id,action,entity_type,entity_id,description,created_at) values(?,?,?,'order.cancel','order',?,?,?)").run(u.business_id,current.branch_id,u.id,String(orderId),`Cancelled unpaid order ${current.order_number}`,stamp);db.prepare("insert into sync_mutations(business_id,client_id,entity_type,entity_id,operation,payload_json,sync_status,created_at,updated_at) values(?,?, 'order',?,'status',?,?,?,?)").run(u.business_id,crypto.randomUUID(),String(orderId),JSON.stringify({order_client_id:current.client_id,status:'cancelled'}),'pending',stamp,stamp)});return p.send({data:orderDto(orderId,u.business_id)});});
  ...
  function appendOrderItems(orderId:number,input:ReturnType<typeof appendOrderSchema.parse>,u:any){
    return tx(()=>{
      const order=one(db,'select * from orders where id=? and business_id=? and (? or branch_id is ?)',orderId,u.business_id,Number(businessWide(u)),u.branch_id);
      ...
      db.prepare("insert into audit_logs(business_id,branch_id,user_id,action,entity_type,entity_id,description,created_at) values(?,?,?,'order.append','order_append',?,?,?)").run(u.business_id,order.branch_id,u.id,input.client_id,`Appended ${lines.length} item line(s) to order ${order.order_number}`,stamp);db.prepare("insert into sync_mutations(business_id,client_id,entity_type,entity_id,operation,payload_json,sync_status,created_at,updated_at) values(?,?, 'order',?,'append',?,?,?,?)").run(u.business_id,input.client_id,String(orderId),JSON.stringify({order_client_id:order.client_id,items:input.items}), 'pending',stamp,stamp);
  ...
  app.get('/api/orders',{preHandler:guard('pos')},async r=>{const u=user(r);return {data:all(db,'select id from orders where business_id=? and (? or branch_id is ?) order by created_at desc limit 250',u.business_id,Number(businessWide(u)),u.branch_id).map(o=>orderDto(o.id,u.business_id))}});
  ...
  app.post('/api/tables',{preHandler:guard('tables')},async(r,p)=>{const u=user(r),v=tableSchema.parse(r.body),token=crypto.randomBytes(32).toString('base64url'),stamp=now(),tableClientId=crypto.randomUUID();if(!one(db,'select 1 ok from rooms where id=? and business_id=?',v.room_id,u.business_id))return p.code(422).send({message:'Room not found.'});const x=db.prepare('insert into restaurant_tables(business_id,branch_id,room_id,table_number,name,capacity,status,qr_token_hash,qr_token_hint,qr_public_token,active,client_id,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(u.business_id,u.branch_id,v.room_id,v.table_number,v.name,v.capacity,v.status,hash(token),token.slice(-8),token,Number(v.active),tableClientId,stamp,stamp);return p.code(201).send({data:{...one(db,'select * from restaurant_tables where id=?',Number(x.lastInsertRowid)),qr_token:token}})});
  ...
  app.get('/api/table-events',{preHandler:guard('tables')},async r=>({data:all(db,"select e.*,t.name table_name,t.table_number from table_events e join restaurant_tables t on t.id=e.table_id where e.business_id=? and (? or t.branch_id is ?) and e.status='pending' order by e.created_at",user(r).business_id,Number(businessWide(user(r))),user(r).branch_id)}));
  app.patch('/api/table-events/:id/resolve',{preHandler:guard('tables')},async(r,p)=>{const u=user(r),eventId=Number((r.params as any).id),stamp=now(),result=db.prepare("update table_events set status='resolved',resolved_by_user_id=?,resolved_at=? where id=? and business_id=? and status='pending' and (? or table_id in(select id from restaurant_tables where branch_id is ?))").run(u.id,stamp,eventId,u.business_id,Number(businessWide(u)),u.branch_id);if(!result.changes)return p.code(404).send({message:'Request not found.'});const ev=one(db,'select * from table_events where id=?',eventId);db.prepare("insert into sync_mutations(business_id,client_id,entity_type,entity_id,operation,payload_json,sync_status,created_at,updated_at) values(?,?, 'table_event',?,'resolve',?,?,?,?)").run(u.business_id,crypto.randomUUID(),String(eventId),JSON.stringify({event_client_id:ev.client_id,status:'resolved'}),'pending',stamp,stamp);return {data:ev}});
  app.get('/api/kitchen/orders',{preHandler:guard('kitchen')},async r=>({data:all(db,"select id from orders where business_id=? and (? or branch_id is ?) and type<>'retail' and status not in('completed','cancelled') order by created_at",user(r).business_id,Number(businessWide(user(r))),user(r).branch_id).map(o=>orderDto(o.id,user(r).business_id))}));
  app.patch('/api/kitchen/items/:id',{preHandler:guard('kitchen')},async(r,p)=>{const u=user(r),status=(r.body as any)?.status;if(!['pending','accepted','preparing','ready','served','cancelled'].includes(status))return p.code(422).send({message:'Invalid status.'});const itemId=Number((r.params as any).id),item=one(db,'select oi.*,o.client_id order_client_id from order_items oi join orders o on o.id=oi.order_id where oi.id=? and o.business_id=? and (? or o.branch_id is ?)',itemId,u.business_id,Number(businessWide(u)),u.branch_id);if(!item)return p.code(404).send({message:'Order item not found.'});db.prepare('update order_items set preparation_status=?,updated_at=? where id=?').run(status,now(),itemId);db.prepare("insert into sync_mutations(business_id,client_id,entity_type,entity_id,operation,payload_json,sync_status,created_at,updated_at) values(?,?, 'order_item',?,'kitchen',?,?,?,?)").run(u.business_id,crypto.randomUUID(),String(itemId),JSON.stringify({order_client_id:item.order_client_id,item_id:itemId,item_index:Number(one(db,'select count(*) n from order_items where order_id=? and id<?',item.order_id,itemId)?.n??0),status}), 'pending',now(),now());const states=all(db,'select preparation_status from order_items where order_id=? and preparation_status is not null',item.order_id).map(row=>row.preparation_status),orderStatus=states.every(value=>value==='served'||value==='cancelled')?'served':states.every(value=>['ready','served','cancelled'].includes(value))?'ready':states.some(value=>['accepted','preparing'].includes(value))?'preparing':'pending';db.prepare('update orders set status=?,updated_at=? where id=?').run(orderStatus,now(),item.order_id);return {data:{...item,preparation_status:status}}});
  ...
  app.post('/api/purchases',{preHandler:guard('purchases')},async(r,p)=>{const u=user(r),v=purchaseSchema.parse(r.body),subtotal=v.items.reduce((s,i)=>s+i.quantity*i.purchase_price,0),stamp=now(),syncId=crypto.randomUUID(),clientId=crypto.randomUUID();const row=tx(()=>{const x=db.prepare('insert into purchases(business_id,branch_id,supplier_id,reference,status,subtotal_cents,tax_cents,total_cents,payment_status,sync_id,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?)').run(u.business_id,u.branch_id,v.supplier_id,v.reference,v.status,cents(subtotal),cents(v.tax),cents(subtotal+v.tax),v.payment_status,syncId,stamp,stamp),id=Number(x.lastInsertRowid);for(const i of v.items)db.prepare('insert into purchase_items(purchase_id,product_id,quantity,purchase_price_cents,tax_cents,total_cents) values(?,?,?,?,?,?)').run(id,i.product_id,i.quantity,cents(i.purchase_price),cents(i.tax),cents(i.quantity*i.purchase_price+i.tax));db.prepare("insert into sync_mutations(business_id,client_id,entity_type,entity_id,operation,payload_json,sync_status,created_at,updated_at) values(?,?,'purchase',?,'create','{}','pending',?,?)").run(u.business_id,clientId,String(id),stamp,stamp);return one(db,'select * from purchases where id=?',id)});return p.code(201).send({data:row})});
  app.post('/api/purchases/:id/receive',{preHandler:guard('purchases')},async(r,p)=>{const u=user(r),purchaseId=Number((r.params as any).id),purchase=one(db,"select * from purchases where id=? and business_id=? and status not in('received','cancelled')",purchaseId,u.business_id);if(!purchase)return p.code(409).send({message:'Purchase cannot be received.'});const result=tx(()=>{const stamp=now();for(const item of all(db,'select * from purchase_items where purchase_id=?',purchaseId)){const product=one(db,'select * from products where id=? and business_id=?',item.product_id,u.business_id),quantity=Number(item.quantity)-Number(item.received_quantity);if(quantity<=0)continue;const before=Number(product.stock),after=before+quantity,eventId=crypto.randomUUID();db.prepare('update products set stock=?,purchase_price_cents=?,updated_at=? where id=?').run(after,item.purchase_price_cents,stamp,item.product_id);db.prepare("insert into stock_movements(business_id,branch_id,product_id,user_id,type,quantity,before_stock,after_stock,note,created_at,updated_at,client_id) values(?,?,?,?,'purchase',?,?,?,?,?,?,?)").run(u.business_id,u.branch_id,item.product_id,u.id,quantity,before,after,`Purchase ${purchase.reference}`,stamp,stamp,eventId);db.prepare('update purchase_items set received_quantity=quantity,stock_client_id=? where id=?').run(eventId,item.id)}db.prepare("update purchases set status='received',received_at=?,updated_at=? where id=?").run(stamp,stamp,purchaseId);db.prepare("insert into sync_mutations(business_id,client_id,entity_type,entity_id,operation,payload_json,sync_status,created_at,updated_at) values(?,?,'purchase',?,'receive','{}','pending',?,?)").run(u.business_id,crypto.randomUUID(),String(purchaseId),stamp,stamp);return one(db,'select * from purchases where id=?',purchaseId)});return {data:result}});
  ...
  registerLocalOrderSync(app,db,guard(),user,tx);
  ...
  app.get('/api/sync/outbox',{preHandler:guard()},async r=>({data:all(db,"select * from sync_mutations where business_id=? and entity_type not in ('branches','settings','categories','units','products','product_variants','product_modifiers','customers','suppliers','users','stock_movement','sale','sale_return','order','order_item','table_event') and sync_status in('pending','failed') order by id limit 100",user(r).business_id).map(x=>({...x,payload:JSON.parse(x.payload_json)}))}));
  ...
  // signing key remains in the Tauri credential store and the Business is
  // resolved by the hosted endpoint from that signature.
  app.get('/api/sync/config',{preHandler:guard()},async()=>({data:{server_url:licenseServerUrl()}}));
  app.get('/api/sync/cash-register/state',{preHandler:guard()},async()=>({data:{cursor:one(db,"select value from sync_state where key='cash_register_cursor'")?.value??null}}));
  app.post('/api/sync/cash-register/ack',{preHandler:guard()},async(r,p)=>{const u=user(r),input=z.object({client_id:z.string().uuid(),local_session_id:z.number().int().positive(),server_session_id:z.number().int().positive()}).parse(r.body);const row=one(db,'select id from cash_register_sessions where id=? and business_id=?',input.local_session_id,u.business_id);if(!row)return p.code(404).send({message:'Local cash session not found.'});tx(()=>{db.prepare("update cash_register_sessions set server_id=?,sync_status='synced',updated_at=? where id=? and business_id=?").run(input.server_session_id,now(),input.local_session_id,u.business_id);db.prepare("update sync_mutations set sync_status='synced',last_error=null,updated_at=? where business_id=? and client_id=?").run(now(),u.business_id,input.client_id);for(const pending of all(db,"select id,payload_json from sync_mutations where business_id=? and entity_type='cash_register_session' and entity_id=? and operation='close' and sync_status in ('pending','failed')",u.business_id,String(input.local_session_id))){const payload=JSON.parse(pending.payload_json);if(!payload.server_session_id)db.prepare('update sync_mutations set payload_json=?,updated_at=? where id=?').run(JSON.stringify({...payload,server_session_id:input.server_session_id}),now(),pending.id)}});return{message:'Acknowledged.'}});
  app.post('/api/sync/cash-register/conflict',{preHandler:guard()},async(r,p)=>{const u=user(r),input=z.object({client_id:z.string().uuid(),local_session_id:z.number().int().positive()}).parse(r.body),row=one(db,"select id from sync_mutations where business_id=? and client_id=? and entity_type='cash_register_session' and entity_id=? and operation='open'",u.business_id,input.client_id,String(input.local_session_id));if(!row)return p.code(404).send({message:'Local cash open mutation not found.'});tx(()=>{const stamp=now();db.prepare("update cash_register_sessions set sync_status='conflict',updated_at=? where id=? and business_id=?").run(stamp,input.local_session_id,u.business_id);db.prepare("update sync_mutations set sync_status='conflict',last_error='CASH_REGISTER_ALREADY_OPEN',updated_at=? where business_id=? and client_id=?").run(stamp,u.business_id,input.client_id);db.prepare("update sync_mutations set sync_status='conflict',last_error='CASH_REGISTER_OPEN_CONFLICT_DEPENDENCY',updated_at=? where business_id=? and entity_type='cash_register_session' and entity_id=? and operation='close' and sync_status in ('pending','failed')").run(stamp,u.business_id,String(input.local_session_id))});return{message:'Conflict recorded.'}});
  app.post('/api/sync/cash-register/defer',{preHandler:guard()},async(r,p)=>{const u=user(r),input=z.object({client_id:z.string().uuid(),local_session_id:z.number().int().positive()}).parse(r.body),row=one(db,"select id from sync_mutations where business_id=? and client_id=? and entity_type='cash_register_session' and entity_id=? and operation='close'",u.business_id,input.client_id,String(input.local_session_id));if(!row)return p.code(404).send({message:'Local cash close mutation not found.'});tx(()=>{const stamp=now();db.prepare("update cash_register_sessions set sync_status='conflict',updated_at=? where id=? and business_id=?").run(stamp,input.local_session_id,u.business_id);db.prepare("update sync_mutations set sync_status='conflict',last_error='SYNC_OPEN_PENDING',updated_at=? where business_id=? and entity_type='cash_register_session' and entity_id=? and operation='close' and sync_status in ('pending','failed')").run(stamp,u.business_id,String(input.local_session_id))});return{message:'Deferred cash close quarantined.'}});
  type RemoteCashSession={id:number;branch_code:string|null;business_date:string;status:'open'|'closed';opened_at:string;opening_cash:number;opening_note:string|null;closed_at:string|null;actual_cash:number|null;closing_note:string|null;updated_at:string};
  const remoteCashSchema=z.object({cursor:z.string().max(160).nullable().optional(),sessions:z.array(z.object({id:z.number().int().positive(),branch_code:z.string().max(40).nullable(),business_date:z.string(),status:z.enum(['open','closed']),opened_at:z.string(),opening_cash:z.number(),opening_note:z.string().nullable(),closed_at:z.string().nullable(),actual_cash:z.number().nullable(),closing_note:z.string().nullable(),updated_at:z.string()})).max(500)});
  const cursorPosition=(value:string|null|undefined)=>{if(!value)return null;const marker=value.lastIndexOf('|'),stamp=marker<0?value:value.slice(0,marker),id=marker<0?0:Number(value.slice(marker+1));return Number.isNaN(Date.parse(stamp))||!Number.isInteger(id)||id<0?null:{stamp,id}};
  const compareCursor=(left:string,right:string)=>{const a=cursorPosition(left),b=cursorPosition(right);if(!a||!b)return 0;const time=Date.parse(a.stamp)-Date.parse(b.stamp);return time||a.id-b.id};
  /** Monotonic remote apply.  Pulled rows are raw SQLite writes only: they do
   * not use the local cash handlers and therefore never create an outbox item. */
  app.post('/api/sync/cash-register/apply-v2',{preHandler:guard()},async(r,p)=>{
    const u=user(r),input=remoteCashSchema.parse(r.body),existingCursor=one(db,"select value from sync_state where key='cash_register_cursor'")?.value as string|undefined;
    if(input.cursor&&existingCursor&&compareCursor(input.cursor,existingCursor)<=0)return{message:'Already reconciled.'};
    tx(()=>{
      const lockedCursor=one(db,"select value from sync_state where key='cash_register_cursor'")?.value as string|undefined;
      if(input.cursor&&lockedCursor&&compareCursor(input.cursor,lockedCursor)<=0)return;
      for(const remote of input.sessions as RemoteCashSession[]){
        const branch=remote.branch_code?one(db,'select id from branches where business_id=? and code=?',u.business_id,remote.branch_code):null;
        if(remote.branch_code&&!branch)throw Object.assign(new Error('Synchronized cash session references an unavailable local branch.'),{statusCode:409,code:'SYNC_BRANCH_NOT_AVAILABLE'});
        const local=one(db,'select * from cash_register_sessions where business_id=? and server_id=?',u.business_id,remote.id);
        if(!local&&remote.status==='open'){
          const conflict=one(db,"select id from cash_register_sessions where business_id=? and branch_id is ? and status='open' and sync_status in ('local','pending')",u.business_id,branch?.id??null);
          if(conflict)db.prepare("update cash_register_sessions set sync_status='conflict',updated_at=? where id=?").run(now(),conflict.id);
        }
        if(local){
          db.prepare("update cash_register_sessions set status=?,branch_id=?,business_date=?,opened_at=?,opening_cash_cents=?,opening_note=?,closed_at=?,actual_cash_cents=?,closing_note=?,sync_status='synced',updated_at=? where id=? and business_id=?").run(remote.status,branch?.id??local.branch_id,remote.business_date,remote.opened_at,cents(remote.opening_cash),remote.opening_note,remote.closed_at,remote.actual_cash===null?null:cents(remote.actual_cash),remote.closing_note,remote.updated_at,local.id,u.business_id);
        }else{
          const owner=one(db,'select id from users where business_id=? and is_active=1 order by id limit 1',u.business_id);
          if(!owner)throw Object.assign(new Error('No local profile exists for synchronized cash session.'),{statusCode:409,code:'SYNC_PROFILE_NOT_AVAILABLE'});
          db.prepare("insert into cash_register_sessions(business_id,branch_id,server_id,sync_status,business_date,status,opened_at,opened_by_user_id,opening_cash_cents,opening_note,closed_at,actual_cash_cents,closing_note,created_at,updated_at) values(?,?,?,'synced',?,?,?,?,?,?,?,?,?,?,?)").run(u.business_id,branch?.id??null,remote.id,remote.business_date,remote.status,remote.opened_at,owner.id,cents(remote.opening_cash),remote.opening_note,remote.closed_at,remote.actual_cash===null?null:cents(remote.actual_cash),remote.closing_note,remote.opened_at,remote.updated_at);
        }
      }
      if(input.cursor)db.prepare("insert into sync_state(key,value,updated_at) values('cash_register_cursor',?,?) on conflict(key) do update set value=excluded.value,updated_at=excluded.updated_at").run(input.cursor,now());
    });
    return{message:'Applied.'};
  });
  app.post('/api/sync/cash-register/apply',{preHandler:guard()},async(r,p)=>{const u=user(r),input=z.object({cursor:z.string().max(160).nullable().optional(),sessions:z.array(z.object({id:z.number().int().positive(),branch_code:z.string().max(40).nullable(),business_date:z.string(),status:z.enum(['open','closed']),opened_at:z.string(),opening_cash:z.number(),opening_note:z.string().nullable(),closed_at:z.string().nullable(),actual_cash:z.number().nullable(),closing_note:z.string().nullable(),updated_at:z.string()})).max(500)}).parse(r.body);tx(()=>{for(const remote of input.sessions){let local=one(db,'select * from cash_register_sessions where business_id=? and server_id=?',u.business_id,remote.id);const branch=remote.branch_code?one(db,'select id from branches where business_id=? and code=?',u.business_id,remote.branch_code):null;if(!local&&remote.status==='open'){const conflict=one(db,"select id from cash_register_sessions where business_id=? and branch_id is ? and status='open' and sync_status in ('local','pending')",u.business_id,branch?.id??null);if(conflict)db.prepare("update cash_register_sessions set sync_status='conflict',updated_at=? where id=?").run(now(),conflict.id)}if(local){db.prepare("update cash_register_sessions set status=?,branch_id=?,business_date=?,opened_at=?,opening_cash_cents=?,opening_note=?,closed_at=?,actual_cash_cents=?,closing_note=?,sync_status='synced',updated_at=? where id=? and business_id=?").run(remote.status,branch?.id??local.branch_id,remote.business_date,remote.opened_at,cents(remote.opening_cash),remote.opening_note,remote.closed_at,remote.actual_cash===null?null:cents(remote.actual_cash),remote.closing_note,remote.updated_at,local.id,u.business_id)}else{const owner=one(db,'select id from users where business_id=? and is_active=1 order by id limit 1',u.business_id);if(!owner)throw Object.assign(new Error('No local profile exists for synchronized cash session.'),{statusCode:409});db.prepare("insert into cash_register_sessions(business_id,branch_id,server_id,sync_status,business_date,status,opened_at,opened_by_user_id,opening_cash_cents,opening_note,closed_at,actual_cash_cents,closing_note,created_at,updated_at) values(?,?,?,'synced',?,?,?,?,?,?,?,?,?,?,?)").run(u.business_id,branch?.id??null,remote.id,remote.business_date,remote.status,remote.opened_at,owner.id,cents(remote.opening_cash),remote.opening_note,remote.closed_at,remote.actual_cash===null?null:cents(remote.actual_cash),remote.closing_note,remote.opened_at,remote.updated_at)}}if(input.cursor)db.prepare("insert into sync_state(key,value,updated_at) values('cash_register_cursor',?,?) on conflict(key) do update set value=excluded.value,updated_at=excluded.updated_at").run(input.cursor,now())});return{message:'Applied.'}});
  ...
  app.post('/api/public/menu/:slug/table/:token/events',async(r,p)=>{const q=r.params as any,t=table(q.slug,q.token),type=(r.body as any)?.type;if(!t)return p.code(404).send({message:'Menu not found.'});if(!['call_waiter','request_bill'].includes(type))return p.code(422).send({message:'Invalid event.'});const eventClientId=crypto.randomUUID(),eventStamp=now(),eventRow=db.prepare('insert into table_events(business_id,table_id,type,status,client_id,created_at) values(?,?,?,\'pending\',?,?)').run(t.business_id,t.id,type,eventClientId,eventStamp);db.prepare("insert into sync_mutations(business_id,client_id,entity_type,entity_id,operation,payload_json,sync_status,created_at,updated_at) values(?,?, 'table_event',?,'create',?,?,?,?)").run(t.business_id,eventClientId,String(eventRow.lastInsertRowid),JSON.stringify({event_client_id:eventClientId,table_id:t.id,type}),'pending',eventStamp,eventStamp);if(type==='request_bill')db.prepare("update restaurant_tables set status='bill_requested',updated_at=? where id=?").run(now(),t.id);return p.code(202).send({message:'Request received.'})});
}
