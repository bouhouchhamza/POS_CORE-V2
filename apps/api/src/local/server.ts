...
export async function buildLocalApp(options: LocalServerOptions = {}) {
  ...
  app.get("/api/cash-register/current", { preHandler: authenticate }, async () => ({ data: mapCashRegisterSession(queryOne(db, "SELECT * FROM cash_register_sessions WHERE status='open' AND sync_status!='conflict' LIMIT 1")) }));
