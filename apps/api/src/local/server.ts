...
export async function buildLocalApp(options: LocalServerOptions = {}) {
  ...
  app.post("/api/cash-register/open", { preHandler: cashManager }, async (request, reply) => { const input = z.object({ opening_cash: z.coerce.number().finite().nonnegative().default(0), opening_note: z.string().trim().max(500).nullable().optional() }).parse(request.body); return immediate(db, () => { if (queryOne(db, "SELECT id FROM cash_register_sessions WHERE status='open'")) return reply.code(422).send({ message: "La caisse est déjà ouverte." }); const timestamp = now(); const result = db.prepare("INSERT INTO cash_register_sessions(business_date,status,opened_at,opened_by_user_id,opening_cash_cents,opening_note,created_at,updated_at) VALUES(?,'open',?,?,?,?,?,?)").run(localDate(), timestamp, request.user.sub, cents(input.opening_cash), input.opening_note ?? null, timestamp, timestamp); return { data: mapCashRegisterSession(queryOne(db, "SELECT * FROM cash_register_sessions WHERE id=?", Number(result.lastInsertRowid))) }; }); });
