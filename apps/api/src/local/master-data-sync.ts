...
function write(db:DatabaseSync,entity:Entity,businessId:number,data:any,syncId:string,serverId:number,updatedAt:string){
  ...
  if(entity==='users'){
    ...
    const values=[category,data.name,data.sku??null,data.barcode??null,data.unit??'piece',Number(data.tax_rate??0),cents(data.purchase_price),cents(data.sale_price),Number(data.min_stock??0),Number(data.track_stock),data.image??null,Number(data.is_active),Number(data.is_public),Number(data.available),stamp];
    if(id)db.prepare('update products set category_id=?,name=?,sku=?,barcode=?,unit=?,tax_rate=?,purchase_price_cents=?,sale_price_cents=?,min_stock=?,track_stock=?,image=?,is_active=?,is_public=?,available=?,updated_at=? where id=? and business_id=?').run(...values,id,businessId);
    else id=Number(db.prepare('insert into products(business_id,category_id,name,sku,barcode,unit,tax_rate,purchase_price_cents,sale_price_cents,stock,min_stock,track_stock,image,is_active,is_public,available,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(businessId,category,data.name,data.sku??null,data.barcode??null,data.unit??'piece',Number(data.tax_rate??0),cents(data.purchase_price),cents(data.sale_price),Number(data.stock??0),Number(data.min_stock??0),Number(data.track_stock),data.image??null,Number(data.is_active),Number(data.is_public),Number(data.available),data.created_at??stamp,stamp).lastInsertRowid);
