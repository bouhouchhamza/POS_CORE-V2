...
test("untracked and mixed-cart sales mutate only tracked stock and rollback atomically", { skip: !enabled }, async()=>{
  ...
  const patronToken=await login();const report=await app.inject({method:'GET',url:'/api/reports/today',headers:auth(patronToken)});assert.equal(report.statusCode,200);assert.equal(report.json().total_products_sold,8);const dashboard=await app.inject({method:'GET',url:'/api/dashboard',headers:auth(patronToken)});assert.equal(dashboard.json().low_stock_count,1);
});
